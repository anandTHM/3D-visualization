  ...
  useEffect(() => {
    ...
    const initViewer = async () => {
      ...
      try {
        ...
        if (!isMounted || currentProjectIdRef.current !== selectedProjects?._id) {
...
useEffect(() => {
  const addDataLayerWithCheck = async () => {
    if (!spaceRef.current || currentProjectIdRef.current !== selectedProjects?._id) {
      return;
    }
    ...
    async (smplr, spaceInstance) => {
      ...
      try {
        ...
        const dataFetchPromise = Promise.all([
          smplrClient.getAllFurnitureInSpace(selectedProjects?.spaceData?.spaceId),
          smplrClient.getSpace(selectedProjects?.spaceData?.spaceId || config.spaceId)
        ]);
